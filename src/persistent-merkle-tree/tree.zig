...
const MAX_NODES_DEPTH = @import("zero_hash.zig").zero_hash_max_depth;
const util = @import("util.zig");
const rootToHex = @import("hex").rootToHex;
const rootIntoHex = @import("hex").rootIntoHex;
...
pub const Tree = struct {
    ...
    pub fn getTreeNodesAtDepth(self: *const Tree, depth: usize, start_index: usize, count: usize, out: []*const Node) !usize {
...
pub fn setNode(pool: *NodePool, root_node: *const Node, gindex: u64, n: *Node) TreeError!*Node {
    var all_bit_array = [_]bool{false} ** MAX_NODES_DEPTH;
    const num_bits = try util.populateBitArray(&all_bit_array, gindex);
    var array_parent_nodes = [_]*const Node{n} ** MAX_NODES_DEPTH;
...
pub fn setNodeAtDepth(pool: *NodePool, root_node: *const Node, nodes_depth: usize, index: usize, node_changed: *Node) !*Node {
    ...
    var nodes_changed = [_]*Node{node_changed};
    return try setNodesAtDepth(pool, root_node, nodes_depth, &indices, &nodes_changed);
}
...
/// Returns the new root node.
fn rebindNodeToRoot(pool: *NodePool, bit_array: []bool, parent_nodes: []*const Node, new_node: *Node) nm.NodeError!*Node {
    var node = new_node;
...
test "should properly navigate the zero tree" {
    ...
    var nodes_arr = [_]*const Node{zero_node} ** depth;
    const nodes = &nodes_arr;

    const count = try tree.getTreeNodesAtDepth(depth, 0, depth, nodes);

    try expect(count == 4);
    for (nodes) |node| {
        const root = nm.getRoot(@constCast(node));
        try std.testing.expectEqualSlices(u8, &zero_root.*, &root.*);
    }
}
...
test "setNode" {
    ...
    // Should compute root correctly after setting a leaf
    {
        ...
        const root_hex = try rootToHex(&root.*);
...
test "Tree batch setNodes" {
    ...
    inline for (&tcs) |tc| {
        ...
        _ = try _getTreeRoots(&tree_ok, max_gindex, &hexes_ok);

        _ = try tree.setTreeNodesAtDepth(depth, &indices, &nodes);

        for (1..max_gindex) |i| {
            const node = try tree.getTreeNode(i);
            const root = nm.getRoot(node);
            var hex = [_]u8{0} ** 66;
            try rootIntoHex(&hex, root);
            try std.testing.expectEqualSlices(u8, &hexes_ok[i - 1], &hex);
        }
    }
}
