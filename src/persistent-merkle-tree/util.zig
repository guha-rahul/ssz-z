...
test "populateBitArray" {
    ...
    for (&tcs) |tc| {
        ...
        const num_bits = try populateBitArray(&bit_array, tc.gindex);
...
test "getByteBoolArray" {
    ...
    try std.testing.expectEqualSlices(bool, &[_]bool{ false, false, false, false, false, true, false, true }, &booleans);
    // 42 = 0b101010
    booleans = getByteBoolArray(42);
    try std.testing.expectEqualSlices(bool, &[_]bool{ false, false, true, false, true, false, true, false }, &booleans);
}
